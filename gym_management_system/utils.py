...
def create_sales_invoice(self):
	...
	si_doc.customer = frappe.db.get_value("Gym Member", self.gym_member, "customer")
	si_doc.posting_date = today()
	si_doc.due_date = add_days(si_doc.posting_date, cint(frappe.db.get_singles_value("Gym Settings", 'default_no_of_days_for_due_date')))
	row  = si_doc.append('items', frappe._dict())
	
	default_item_key = "default_membership_item" if self.doctype == "Gym Membership" else "default_subscription_item"
	default_item = frappe.db.get_singles_value("Gym Settings", default_item_key)
